...
- **Drop files in.** Dragging files onto a tab types their quoted paths — how you hand Claude an image. Linux caveat: winit has no Wayland drop support, so this needs X11; set `behavior.prefer_x11 = true` and Giverny runs under XWayland (softer text at fractional scaling). `giverny doctor` says which you have.
