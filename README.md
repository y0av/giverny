...
- **Drop files in.** Dragging files onto a tab types their quoted paths — how you hand Claude an image. Linux caveat: the winit egui pins delivers drops on X11 only — Wayland support exists in winit master and arrives with egui's winit 0.31 migration. Until then set `behavior.prefer_x11 = true` to run under XWayland (softer text at fractional scaling). `giverny doctor` says which you have.
