...
pub struct State {
    ...
    /// Not upstream: whether the drag in flight is one the application wants
    /// (see `dnd`). Drags of anything but files are ignored, and must not
    /// produce leave or motion reports either.
    dnd_active: bool,
}
...
impl DataDeviceHandler for State {
    // Not upstream: the drag half of the data device (see `dnd`). Upstream
    // leaves enter/leave/motion/drop_performed empty; `selection`, at the
    // bottom, is untouched.
    fn enter(
        &mut self,
        _: &Connection,
        ...
        // Accepting is what makes the source show a "copy" cursor and, in
        // version 3, what makes the drop happen at all. Refuse anything that
        // is not a file list, and refuse everything when nothing is listening.
        self.dnd_active = dnd::wanted()
            && offer.with_mime_types(|types| types.iter().any(|ty| ty == dnd::URI_LIST));
        let mime = self.dnd_active.then(|| dnd::URI_LIST.to_string());
        offer.accept_mime_type(offer.serial, mime);
        let action = if self.dnd_active { DndAction::Copy } else { DndAction::None };
        offer.set_actions(action, action);
        if self.dnd_active {
            dnd::emit(DragEvent::Enter { x, y, surface: surface.id().as_ptr() as usize });
        }
    }

    fn leave(&mut self, _: &Connection, _: &QueueHandle<Self>, _: &WlDataDevice) {
        if mem::take(&mut self.dnd_active) {
            dnd::emit(DragEvent::Leave);
        }
    }
    ...
    fn motion(
        ...
        // Re-state what we accept, the way GTK does: a source that keeps
        // asking must keep hearing yes, or it ends the drag with no drop.
        if let Some(offer) = drag_offer(data_device) {
            offer.accept_mime_type(offer.serial, Some(dnd::URI_LIST.to_string()));
            offer.set_actions(DndAction::Copy, DndAction::Copy);
        }
        dnd::emit(DragEvent::Motion { x, y });
    }

    fn drop_performed(&mut self, _: &Connection, _: &QueueHandle<Self>, data_device: &WlDataDevice) {
        if !mem::take(&mut self.dnd_active) {
            return;
        }
        // The source writes the URI list into a pipe. Read it on the event
        // loop, the way selections are read: blocking here would stall every
        // clipboard request behind a drag source that stops writing.
        let Some(offer) = drag_offer(data_device) else {
            dnd::emit(DragEvent::Leave);
            return;
        };
        let read_pipe = match offer.receive(dnd::URI_LIST.to_string()) {
            Ok(pipe) => pipe,
            Err(_) => {
                dnd::emit(DragEvent::Leave);
                return;
        ...
        let mut reader_buffer = [0; 4096];
        let mut content = Vec::new();
        let inserted = self.loop_handle.insert_source(read_pipe, move |_, file, _| {
            let file = unsafe { file.get_mut() };
            loop {
                match file.read(&mut reader_buffer) {
                    Ok(0) => {
                        dnd::emit(DragEvent::Drop(mem::take(&mut content)));
                        offer.finish();
                        break PostAction::Remove;
                    },
                    // A drop is a handful of paths; a source sending more than
                    // this is not sending a file list.
                    Ok(_) if content.len() > 1 << 20 => {
                        dnd::emit(DragEvent::Leave);
                        break PostAction::Remove;
                    },
                    Ok(n) => content.extend_from_slice(&reader_buffer[..n]),
